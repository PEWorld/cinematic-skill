...
### W5c — Правило разброса крупности и композиции (ант-«всё средний план»)
Боевой провал мятного-топа 2026-07-04: все 6 кадров — «человек по пояс/в рост, по центру, лицом в линзу», одна дистанция камеры → скучно и однообразно, даже когда коллизии есть. Ролевой пул *подразумевает* смену крупности, но генерация схлопывает всё в medium-центр. Жёсткие квоты:
- **≤2 кадра одной крупности.** Три medium-shot подряд = брак.
- **Обязательны все три крайности масштаба в россыпи:**
  - **1+ EXTREME MACRO** — деталь во весь кадр (крест на ключице, hardware/пуговичный ряд, фактура ткани, кольца, руки). Тактильный кадр обязателен.
  - **1+ TRUE WIDE / god-scale** — субъект мелкий, мир огромный (передний план гигантом, горизонт, вертикаль).
  - **1+ WORLD-DEADPAN без субъекта** — мир/коллизия без фигуры (пустой трон, следы, реквизит крупно).
- **Вариация композиции, не только фас-центр:** минимум 2 кадра НЕ симметрично-по-центру-в-линзу — профиль, спина, over-shoulder, off-center, dutch, overhead.
- Матрица крупности как ориентир на 6 кадров: `[macro] · [close/hook] · [medium] · [wide/god-scale] · [medium-alt угол] · [world-deadpan или full-body aftermath]`.
...
### W9 — Правило сломанного лица + ANTI-POSE гейт
Если лицо в кадре — оно НЕ позирует: рычит, ржёт в голос, орёт, застыло в жути, смотрит как зверь ИЛИ deadpan-безразличие (см. W3 non-reaction). Модельная нейтральность и «смелый взгляд в камеру из лукбука» запрещены. Эмоция на лице должна быть неприлично сильной для fashion-съёмки — ИЛИ демонстративно никакой.

⛔ **ANTI-POSE (жёсткий бан, боевой провал мятного-топа 2026-07-04):** запрещены selfie-жесты и кокетство — `peace sign / V-sign, tongue out, wink, coy smile, hand on hip, adjusting hair/top, blowing kiss, cute pose, looking playfully at camera, duck face`. Это мгновенно схлопывает кадр в лукбук/селфи и убивает любую коллизию (мокрый пёс в ванне + peace-sign = селфи, а не «моет собаку в убитой ванне»). Разрешены ТОЛЬКО: слом (крик/оскал/хохот/ужас), deadpan (не реагирует, смотрит мимо), или действие (шаг/поворот/тянет/хватает — W12). Если в промпте нет ни слома, ни deadpan, ни действия — кадр позирует, переписать.
...
### ⚠️ Правило МЯГКОЙ вещи (боевой провал мятного-топа 2026-07-04)
Известный failure mode: агрессивная вещь (гот/техвир/питон) сама провоцирует крик+тотем+коллизию, а **мягкая/феминная/нежная вещь ставит скилл в тупик — и он откатывается в дефолт «красивая девочка позирует в грязном городе»** (city-grit фон + позирование + пастельный грейд = запрещённая instagram-aesthetic зона). Если GARMENT DNA = soft / feminine / delicate / silk / pastel / domestic-soft / cropped-tender:
1. **Коллизия обязана строиться на КОНТРАСТЕ «мягкость × жестокость/неуместность»**, а не на грязном фоне. Нежное = невинный якорь в жестоком/неправильном мире (Mode B): нежный лук моет грязного пса в убитой ванне · стоит невозмутимо в мясном цехе · пьёт чай за накрытым столом в затопленной комнате · идёт мимо мёртвых собак по растрескавшейся земле. Мир жёсткий — она единственное мягкое пятно.
2. **Позирование запрещено ВДВОЙНЕ** (см. ANTI-POSE): нежная вещь тянет модель в кокетство сильнее всего. Только deadpan-невозмутимость или тихое действие.
3. **«Грязный город + модель» — это НЕ коллизия для мягкой вещи.** Сеульский переулок/конбини/неон как фон под позирующую девочку = провал. Нужен именно контраст-мир, а не грязный задник.
...
⛔ **ONE-TOTEM гейт (боевой провал 3 раза подряд: склеп, панельки, Токио — скилл смешивал питона + аллигатора + варана в одной россыпи).** ОДИН вид животного на всю констелляцию, зафиксированный дословно — **вид + цвет + размер** — и скопированный слово-в-слово во все кадры, где тотем есть (как GARMENT LOCK и Style anchor). Пример лока: `albino white Burmese python, 4 meters, thick body` — и только он во всех кадрах. Нельзя «змея» в одном и «крокодил» в другом: MJ подставит разных животных, единство рушится. Смешанные виды в россыпи = брак, пересобрать под один лок.
...
- **Разброс крупности (W5c): обязательны 1 macro + 1 wide/god-scale + 1 world-deadpan; ≤2 кадра одной крупности; ≥2 кадра НЕ фас-по-центру.**
- **Anti-pose (W9): ноль selfie-жестов (peace-sign/язык/кокетство); каждое лицо — слом ИЛИ deadpan ИЛИ действие.**
- **Тотем (ONE-TOTEM гейт): один вид животного, дословный лок вид+цвет+размер во всех кадрах.**
...
- [ ] **Разброс крупности (W5c):** есть 1 macro + 1 wide/god-scale + 1 world-deadpan, ≤2 одной крупности, ≥2 не-фас-по-центру — НЕ все «средний план по центру»
- [ ] **Anti-pose (W9):** ноль peace-sign/языка/кокетства; каждое лицо — слом ИЛИ deadpan ИЛИ действие
- [ ] **Мягкая вещь:** если feminine/soft — коллизия через контраст «мягкость × жёсткость», НЕ «грязный город + модель»
- [ ] **ONE-TOTEM:** один вид животного, дословный лок (вид+цвет+размер) во всех кадрах с тотемом
